...
public final class KailleraUserImpl implements KailleraUser, Executable {
  ...
  public KailleraUserImpl(
      int userID,
      String protocol,
      InetSocketAddress connectSocketAddress,
      KailleraEventListener listener,
  ...
  public synchronized void gameKick(int userID) throws GameKickException {
    ...
    if (game == null) {
      log.warn(
          this + " kick User " + userID + " failed: Not in a game"); // $NON-NLS-1$ //$NON-NLS-2$
      throw new GameKickException(
          EmuLang.getString("KailleraUserImpl.KickErrorNotInGame")); // $NON-NLS-1$
    }
  ...
  public synchronized void quitGame()
    ...
    if (status != KailleraUser.STATUS_IDLE) setStatus(KailleraUser.STATUS_IDLE);
  ...
  public void run() {
    ...
    try {
      while (!stopFlag) {
        ...
        else if (event instanceof StopFlagEvent) break;
