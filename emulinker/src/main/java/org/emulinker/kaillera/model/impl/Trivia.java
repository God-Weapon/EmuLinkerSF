	...
	public void run(){
		...
		while(!exitThread){
			if(!triviaPaused){
				...
				server.announce("<Trivia> " + (questionTime/1000) + " seconds until the next question. Get ready for question " + (count + 1) + " of " + questions.size(), false, null);
		...
		if(message.toLowerCase().equals(questions.get(questions_count).getAnswer().toLowerCase())){
			...
			if(message.toLowerCase().equals(questions.get(questions_count).getAnswer().toLowerCase())){
				...
				for(int i = 0; i < placement0.length; i++){
					...
					else if(questions.get(questions_count).getAnswer().toLowerCase().contains(placement1[i])){
	...
	public void addScore(String nick, String ip, String answer){
		...
		for(int i = 0; i < scores.size(); i++){
			if(scores.get(i).getIP().equals(ip)){
				...
				server.announce("<Trivia> " + nick + " is the winner of this round (" + questions.get(questions_count).getAnswer() + ")! Your score is: " + s, false, null);
				...
				if(ip_streak.equals(ip)){
					...
					if(score_streak > 1){
						...
						server.announce("<Trivia> ***" + nick + " has won " + score_streak + " in a row!***", false, null);
					}
				}
				else{
					score_streak = 1;
				...
				if(s == 25){
					...
					server.announce("<Trivia> " + nick + ", you're so smart you're going to break the Trivia Bot!", false, null);
				}		
				else if(s == 100){
					server.announce("<Trivia> " + nick + ", you're in a league of your own. Nobody can compete!", false, null);
				}
				else if(s % 100 == 0){
		...
		score_streak = 1;
		ip_streak = ip;
		server.announce("<Trivia> " + nick + " is the winner of this round (" + answer + ")! His score is: 1", false, null);
	}
