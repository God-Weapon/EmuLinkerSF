...
public class KailleraServerImpl implements KailleraServer, Executable {
    ...
    log.info(
        user
            ...
            + KailleraUser.CONNECTION_TYPE_NAMES[
                user.getConnectionType()]); // $NON-NLS-4$ //$NON-NLS-5$
  ...
  public synchronized void quit(KailleraUser user, String message)
    ...
    if (users.remove(user.getID()) == null) log.error(user + " quit failed: not in user list");
  ...
  public void run() {
    ...
    try {
      ...
      if (!stopFlag) log.fatal("KailleraServer thread caught unexpected exception: " + e, e);
    } finally {
      isRunning = false;
