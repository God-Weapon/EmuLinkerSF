...
public class V086Bundle extends ByteBufferMessage {
  ...
  public V086Bundle(V086Message[] messages, int numToWrite) {
    ...
    if (numToWrite < this.numToWrite) this.numToWrite = numToWrite;
  ...
  public static V086Bundle parse(ByteBuffer buffer, int lastMessageID)
    ...
    int msgNum =
        buffer.getChar(
            1); // buffer.getShort(1); - mistake. max value of short is 0x7FFF but we need 0xFFFF
    if ((msgNum - 1) == lastMessageID
      ...
      if (messageLength < 2
          || messageLength > buffer.remaining()) // || messageLength > buffer.limit())
