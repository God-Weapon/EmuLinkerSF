...
public class GameDataQueue
{
	...
	public GameDataQueue(int gameID, int numPlayers, int timeoutMillis, int retries)
	{
		...
		for (int i = 0; i < playerQueues.length; i++)
			playerQueues[i] = new PlayerDataQueue((i + 1));
	}
	...
	private class PlayerDataQueue
	{
		...
		private byte[] getData(int byteCount, int bytesPerAction) throws PlayerTimeoutException, DesynchException
		{
			...
			if (lastData != null)
			{
				data = lastData;
				lastData = null;
				//				log.debug("Player " + thisPlayerNumber + ": getData with i=" + lastI + ", j=" + lastJ);
			}
			else
				data = new byte[(byteCount * numPlayers)];
			...
			for (int i = lastI; i < ((byteCount / bytesPerAction) * numPlayers); i++)
			{
				for (int j = lastJ; j < bytesPerAction; j++)
				{
					try
					{
						data[((i * bytesPerAction) + j)] = queues[(i % numPlayers)].get(timeoutMillis, TimeUnit.MILLISECONDS);
					}
					catch (TimeoutException e)
					{
						...
						if (++timeoutCounter > retries)
							throw new DesynchException("Player " + ((i % numPlayers) + 1) + " is lagged!", ((i % numPlayers) + 1), e);
						else
							throw new PlayerTimeoutException(((i % numPlayers) + 1), timeoutCounter, e);
					}
				}
			}
