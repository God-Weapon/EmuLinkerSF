...
public class EmuUtil {
  ...
  public static String byteToHex(byte b) {
    return Character.toString(HEX_CHARS[((b & 0xf0) >> 4)])
        + Character.toString(HEX_CHARS[(b & 0xf)]);
  }
  ...
  public static String readString(ByteBuffer buffer, int stopByte, Charset charset) {
    ...
    {
      byte b;
      if ((b = buffer.get()) == stopByte) break;
      tempBuffer.put(b);
      //			tempArray[i] = (char)b;
    }
    // Cast to avoid issue with java version mismatch: https://stackoverflow.com/a/61267496/2875073
  ...
  public static Object construct(String className, Object args[]) throws InstantiationException {
    try {
      ...
      for (int i = 0; i < args.length; i++) contructorArgs[i] = args[i].getClass();
