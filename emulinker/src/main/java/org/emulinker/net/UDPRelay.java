...
public abstract class UDPRelay implements Runnable
{
	...
	protected Map<InetSocketAddress, ClientHandler> clients = Collections.synchronizedMap(new HashMap<InetSocketAddress, ClientHandler>());
	...
	public DatagramChannel getListenChannel() { return listenChannel; }
	public InetSocketAddress getServerSocketAddress() { return serverSocketAddress; }
		...
		try
		{
			while(true)
			{
				...
				if(clientHandler == null)
				{
					...
					{
						log.error("Failed to start new ClientHandler for " + EmuUtil.formatSocketAddress(clientAddress), e);
						continue;
					}
	...
	protected class ClientHandler implements Runnable
	{
		...
		protected ClientHandler(InetSocketAddress clientSocketAddress) throws Exception
		{
			...
			log.info("ClientHandler for " + EmuUtil.formatSocketAddress(clientSocketAddress) + " bound to port " + clientChannel.socket().getPort());
		}
		
		protected void send(ByteBuffer buffer) throws Exception
		{
//			log.info(EmuUtil.formatSocketAddress(clientSocketAddress) + " -> \t" + EmuUtil.dumpBuffer(buffer));
			ByteBuffer newBuffer = processClientToServer(buffer, clientSocketAddress, serverSocketAddress);
			clientChannel.send(newBuffer, serverSocketAddress);
		}
		
		public void run()
		{
			log.info("ClientHandler thread for " + EmuUtil.formatSocketAddress(clientSocketAddress) + " runnning...");
...
//					log.info(EmuUtil.formatSocketAddress(clientSocketAddress) + " <- \t" + EmuUtil.dumpBuffer(buffer));
			...
			{
				log.info("ClientHandler thread for " + EmuUtil.formatSocketAddress(clientSocketAddress) + " caught exception: " + e.getMessage(), e);
			}
			finally
			{
				try
			...
			log.info("ClientHandler thread for " + EmuUtil.formatSocketAddress(clientSocketAddress) + " exiting...");
		}
	}
}
