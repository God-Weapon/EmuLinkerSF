...
public class KailleraRelay extends UDPRelay
{
	...
	protected ByteBuffer processClientToServer(ByteBuffer receiveBuffer, InetSocketAddress fromAddress, InetSocketAddress toAddress)
	{
		...
		log.debug(EmuUtil.formatSocketAddress(fromAddress) + " -> " + EmuUtil.formatSocketAddress(toAddress) + ": " + inMessage);
	...
	protected ByteBuffer processServerToClient(ByteBuffer receiveBuffer, InetSocketAddress fromAddress, InetSocketAddress toAddress)
	{
		ConnectMessage inMessage = null;
		...
		if (inMessage instanceof ConnectMessage_HELLOD00D)
		{
			...
			try
			{
				new V086Relay(portMsg.getPort(), new InetSocketAddress(getServerSocketAddress().getAddress(), portMsg.getPort()));
			}
			catch (Exception e)
			{
